...
async def entry(bot: commands.Bot, *args: typing.Any, **kwargs: typing.Any):
    ...
    async with bot:
        ...
        for extension in bot.extensions_to_load:  # type: ignore[reportAttributeAccessIssue]
            extension: str
        ...
        try:
            import pyperclip  # type: ignore[reportMissingModuleSource] # pylint: disable=import-outside-toplevel
        except ImportError:
            LOGGER.critical(
        ...
        if not bot.skip_wait:  # type: ignore[reportAttributeAccessIssue]
...
def entrypoint(
    ...
    def prefix(bot: commands.Bot, _: discord.Message) -> typing.List[str]:
        return [
            f"{bot.unique_id}::",  # type: ignore[reportAttributeAccessIssue]
            f"<@{bot.user.id}> {bot.unique_id}::",  # type: ignore[reportAttributeAccessIssue]
            f"<@!{bot.user.id}> {bot.unique_id}::",  # type: ignore[reportAttributeAccessIssue]
        ]

    bot = commands.Bot(prefix, intents=intents_class)
    bot.unique_id = str(uuid.uuid4())  # type: ignore[reportAttributeAccessIssue]
    bot.extensions_to_load = load_extension  # type: ignore[reportAttributeAccessIssue]
    bot.skip_wait = skip_wait  # type: ignore[reportAttributeAccessIssue]
