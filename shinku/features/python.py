...
try:
    import line_profiler  # type: ignore[reportMissingImport]
except ImportError:
    line_profiler = None
...
class PythonFeature(Feature):
    ...
    async def jsk_retain(self, ctx: ContextA, *, toggle: bool = None):  # type: ignore[reportArgumentType]
        ...
        if toggle is None:  # type: ignore[reportUnnecessaryComparison]
            if self.retain:
                return await ctx.send("Variable retention is set to ON.")  # type: ignore[reportUnreachable]
        ...
        try:
            async with ReplResponseReactor(ctx.message):
                with self.submit(ctx):
                    ...
                    async for send, result in AsyncSender(executor):  # type: ignore[reportArgumentType]
            ...
            try:
                async with ReplResponseReactor(ctx.message):
                    with self.submit(ctx):
                        ...
                        line_timings: typing.Dict[int, typing.List[float]] = (
                            collections.defaultdict(list)
                        )

                        while (
                            count < 10_000
                            and (time.perf_counter() - overall_start) < 30.0
                        ):
                            profile = line_profiler.LineProfiler()  # type: ignore[reportOptionalMemberAccess]
                            ...
                            for function in profile.code_map.values():
                                for timing in function.values():
                                    line_timings[timing["lineno"]].append(
                                        timing["total_time"] * profile.timer_unit
                                    )
                                    ioless_time += (
                                        timing["total_time"] * profile.timer_unit
                                    )
                        ...
                        max_line_time = max(
                            max(timing) for timing in line_timings.values()
                        )
    ...
    async def jsk_ast(self, ctx: ContextA, *, argument: codeblock_converter):  # type: ignore[reportGeneralTypeIssue]
        ...
        async with ReplResponseReactor(ctx.message):
            ...
            await ctx.send(
                file=discord.File(
                    filename="ast.ansi", fp=io.BytesIO(text.encode("utf-8"))
                )
            )
